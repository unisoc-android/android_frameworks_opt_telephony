...
public class IccSmsInterfaceManagerEx extends IccSmsInterfaceManager {
    ...
    protected Handler mHandlerEx = new Handler() {
        @Override
    ...
    public int copyMessageToIccEfSprd(String callingPackage, int status, byte[] pdu, byte[] smsc) {
        Log.d(TAG, "copyMessageToIccEfSprd: status=" + status + " ==> " +
                "pdu=("+ Arrays.toString(pdu) +
                "), smsc=(" + Arrays.toString(smsc) +")");
        enforceReceiveAndSend("Copying message to Icc");
        if (mAppOps.noteOp(AppOpsManager.OP_WRITE_ICC_SMS, Binder.getCallingUid(),
                callingPackage) != AppOpsManager.MODE_ALLOWED) {
            Log.d(TAG, "copyMessageToIccEfSprd mAppOps.noteOp AppOpsManager.MODE_ALLOWED");
            return -1;
        }
        synchronized(mLock) {
            mSuccess = false;
            mIndex = -1;
            Message response = mHandlerEx.obtainMessage(EVENT_UPDATE_DONE_SPRD);

            //RIL_REQUEST_WRITE_SMS_TO_SIM vs RIL_REQUEST_CDMA_WRITE_SMS_TO_RUIM
            Log.d(TAG, "copyMessageToIccEfSprd mPhone.getPhoneType:"+mPhone.getPhoneType());
            ...
            try {
                ...
                Log.d(TAG, "copyMessageToIccEfSprd interrupted while trying to update by index");
