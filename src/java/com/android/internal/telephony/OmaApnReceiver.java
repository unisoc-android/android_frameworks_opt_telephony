...
public class OmaApnReceiver extends BroadcastReceiver {
    ...
    private String mPreferredApn = null;
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(INTENT_APN_DATA_CONFIG)) {
            ...
            Cursor cursor = mContext.getContentResolver().query(
                    Telephony.Carriers.CONTENT_URI, new String[] { "_id" },where.toString(), null, null);
            try {
                if (cursor != null && cursor.getCount() > 0) {
                    if (cursor.moveToFirst()) {
                        do {
                            mOriginalApnMap.put(cursor.getString(ID_INDEX),"haveNotOverlay");
    ...
    void ProcessIntent(Intent intent) {
        ...
        for (int i = 1; i <= apnSize; i++) {
            Log.d(TAG,"apn" + i + " bundle: " + ((Bundle) intent.getExtra("apn" + i)).toString());
        ...
        for (int i = 1; i <= bootstarpSize; i++) {
            Log.d(TAG,"bootstarp" + i + " bundle: " + (Bundle) intent.getExtra("bootstarp" + i));
            ProcessBundle((Bundle) intent.getExtra("bootstarp" + i),bundleTypes[2]);
        }
    }
    ...
    void ProcessBundle(Bundle bundle, String bundleType) {
        if (bundle != null) {
            ...
            if (bundle.getString("nap-address") != null
                ...
                validAndPut(contentValues, Telephony.Carriers.NUMERIC, null, numeric);
                validAndPut(contentValues, Telephony.Carriers.MCC, null, mcc);
                validAndPut(contentValues, Telephony.Carriers.MNC, null, mnc);
                validAndPut(contentValues, Telephony.Carriers.APN, bundle,"nap-address");

                Log.d(TAG,"key = authtype , value = " + bundle.getString("authtype"));
                ...
                if (appSize > 0) {
                    for (int i = 1; i <= appSize; i++) {
                        bundleMatch(i,contentValues,bundle,numeric,uri);
                    }
                } else {
    ...
    public void bundleMatch(int order,ContentValues contentValues,Bundle bundle,String numeric,Uri uri) {
        Bundle appBundle = (Bundle) mIntent.getExtra("app" + order);
        Log.d(TAG, "[appBundle" + order + "]=[" + appBundle.toString() + "]");
        if (appBundle.getString("to-proxy") != null) {
            if (appBundle.getString("to-proxy").equals(bundle.getString("proxy-id"))) {
                if (appBundle.getString("appid").equals("w4")) {
                    // APN type mms
                    Log.d(TAG,"appBundle appid = "+ appBundle.getString("appid") + " turn to mms type");
                    putContentValuesNull(contentValues);
                    validAndPut(contentValues,Telephony.Carriers.TYPE, null,"mms");
                    validAndPut(contentValues,Telephony.Carriers.MMSC, appBundle,"addr");
                    validAndPut(contentValues,Telephony.Carriers.MMSPROXY,bundle, "pxaddr");
                    validAndPut(contentValues,Telephony.Carriers.MMSPORT, bundle,"portnbr");
                    if (bundle.getString("napdef_name") != null) {
                        validAndPut(contentValues,Telephony.Carriers.NAME,bundle, "napdef_name");
                    } else {
                        validAndPut(contentValues,Telephony.Carriers.NAME,appBundle, "application_name");
                    }
                    if (bundle.getString("pxauth-id") != null) {
                        validAndPut(contentValues,Telephony.Carriers.USER,bundle, "pxauth-id");
                    } else {
                        validAndPut(contentValues,Telephony.Carriers.USER,bundle, "authname");
                    }
                    if (bundle.getString("pxauth-pw") != null) {
                        validAndPut(contentValues,Telephony.Carriers.PASSWORD,bundle, "pxauth-pw");
                    } else {
                        validAndPut(contentValues,Telephony.Carriers.PASSWORD,bundle, "authsecret");
                    }
                    putValuesToDatabase(contentValues, numeric,uri);
                } else {
                    // APN type default
                    Log.d(TAG,"appBundle appid = " + appBundle.getString("appid") + " turn to default type");
                    putDefaultApnValues(bundle, contentValues,numeric, uri);
                }
            } else {
                Log.d(TAG, "appBundle" + order + " to-proxy = "
            ...
            if (appBundle.getString("to-napid").equals(bundle.getString("napid"))) {
                ...
                Log.d(TAG, "appBundle" + order + " to-napid = "
    ...
    public void putDefaultApnValues(Bundle apnBundle, ContentValues contentValues, String numeric,Uri uri) {
        Log.d(TAG, "putDefaultApnValues ");
        putContentValuesNull(contentValues);
        validAndPut(contentValues, Telephony.Carriers.TYPE, null,"default");
        validAndPut(contentValues, Telephony.Carriers.NAME, apnBundle,"napdef_name");
        validAndPut(contentValues, Telephony.Carriers.USER, apnBundle,"authname");
        validAndPut(contentValues, Telephony.Carriers.PASSWORD,apnBundle, "authsecret");
        validAndPut(contentValues, Telephony.Carriers.PROXY,apnBundle, "pxaddr");
        validAndPut(contentValues, Telephony.Carriers.PORT, apnBundle,"portnbr");
        putValuesToDatabase(contentValues, numeric, uri);
    }
    ...
    public void putValuesToDatabase(ContentValues contentValues, String numeric, Uri uri) {
        ...
        try {
            ...
            if (contentValues.get(Telephony.Carriers.TYPE) != null
                ...
                overlayOrAddApn((String) contentValues.get(Telephony.Carriers.TYPE),
    ...
    public void validAndPut(ContentValues contentValues, String apnKey,
        ...
        if (bundle != null) {
            Log.d(TAG,"validAndPut: apnKey = " + apnKey + "  bundleKey = " + bundleKey);
            ...
            Log.d(TAG, "validAndPut: apnKey = " + apnKey + " apnValue = " + bundleKey);
            if (bundleKey != null) {
    ...
    public void setPreferredApn(Uri result,ContentResolver resolver, ContentValues contentValues) {
        boolean isContainDefault = false;
        boolean isLatestAsPrefferred = Resources.getSystem().getBoolean(com.android.internal.R.bool.mark_latest_as_preferred);
        String apnType = contentValues.getAsString(Telephony.Carriers.TYPE);
        String apn = contentValues.getAsString(Telephony.Carriers.APN);
        Log.d(TAG,"isLatestAsPrefferred :" + isLatestAsPrefferred);
        if (apnType != null && apnType.indexOf("default") != -1) {
            isContainDefault= true;
        }
        if (isContainDefault && isLatestAsPrefferred){
            long apnId;
            apnId = ContentUris.parseId(result);
            Uri uri = Uri.withAppendedPath(PREFERAPN_NO_UPDATE_URI_USING_SUBID, String.valueOf(mSubId));

            if (apnId >= 0) {
                Log.d(TAG,"setPreferredApn: delete uri:" + uri.toString());
                resolver.delete(uri, null, null);
                Log.d(TAG,"setPreferredApn: insert apnId" + apnId);
                ContentValues values = new ContentValues();
                values.put(APN_ID, apnId);
                resolver.insert(uri, values);
                mPreferredApn = apn;
                Log.d(TAG,"setPreferredApn: mPreferredApn is " + apn);
            }
        }
    }
            ...
            if (cursor != null && cursor.moveToFirst()) {
                ...
                do {
                    ...
                    if ("mms".equalsIgnoreCase(apnType)) {
                        ...
                        String queriedApn = mOriginalApnMap.get(cursor.getString(ID_INDEX));
                        ...
                        if (queriedApn != null) {
                            if ("haveNotOverlay".equals(queriedApn)) {// overlay
                                                                      // method
                                                                      // one
                                if (cursor.getString(NAME_INDEX) != null&& cursor
                                    ...
                                    if (count == 0) {
                                        result = resolver.insert(uri, contentValues);
                                        count++;
                                        setPreferredApn(result,resolver,contentValues);
                                    }
                                    if (mOriginalApnMap.get(cursor.getString(ID_INDEX)) != null) {
                                        mOriginalApnMap.put(cursor.getString(ID_INDEX),"haveOverlay");
                                    }
                                    if (result != null) {
                                        whereId.append(cursor.getString(ID_INDEX)).append("'");
                                ...
                                Log.d(TAG,"should not happen! this apn has being deleted");
                            }
                        } else {// new apn
                            Log.d(TAG,"cursor.apn = "
                                            + cursor.getString(APN_INDEX)
                                            + "contentValues.apn = "
                                            + contentValues.getAsString("apn")
                                            + "match = "
                                            + (cursor.getString(APN_INDEX).equals(contentValues.getAsString("apn"))));
                            if (!(cursor.getString(APN_INDEX).equals(contentValues.getAsString(Telephony.Carriers.APN)))) {
                                if (count == 0) {
                                    result = resolver.insert(uri, contentValues);
                                    count++;
                                    setPreferredApn(result,resolver,contentValues);
                                ...
                                if (count == 0) {
                                    ...
                                    if (mPreferredApn != null &&
                                            mPreferredApn.equals(contentValues.getAsString(Telephony.Carriers.APN))) {
                                        setPreferredApn(result,resolver,contentValues);
                                    }
                                }
                                if (result != null) {
