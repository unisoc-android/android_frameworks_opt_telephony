...
public class AdnRecordCacheEx extends AdnRecordCache implements IccConstantsEx {
    ...
    public void requestLoadAllAdnLike(int efid, int extensionEf,
        ...
        if (result != null) {
            ...
            log("Have already loaded ef: " + efid);
            return;
        }
        ...
        if (waiters != null) {
            ...
            log("Already started loading this efid: " + efid);
            return;
        }
    ...
    private boolean isCleanRecord(int num, int type, AdnRecordEx oldAdn,
        ...
        for (i = 0; i < count; i++) {
            //UNISOC: add for 1138289, update sne,Null pointer exception.
            ...
            if (i < oldCount && aasArr1 != null && i < aasArr1.length && aasArr1[i] != null) {
                aas1 = aasArr1[i];
            } else {
                aas1 = "";
            }

            if (i < newCount && aasArr2 != null && i < aasArr2.length && aasArr2[i] != null) {
                aas2 = aasArr2[i];
            } else {
                aas2 = "";
            }
    ...
    private boolean compareSubject(int type, AdnRecordEx oldAdn, AdnRecordEx newAdn) {
        ...
        case UsimPhoneBookManagerEx.USIM_SUBJCET_GRP:
            log("oldAdn.mGrp == " + oldAdn.mGrp + ", newAdn.mGrp == " + newAdn.mGrp);
        ...
        case UsimPhoneBookManagerEx.USIM_SUBJCET_SNE:
            log("oldAdn.mSne == " + oldAdn.mSne + ", newAdn.mSne == " + newAdn.mSne);
    ...
    private int[] getUpdateSubjectFlag(int num, int type, AdnRecordEx oldAdn,
        ...
        String[] strArr1 = getSubjectString(type, oldAdn);
        String[] strArr2 = getSubjectString(type, newAdn);
        String[] aasArr1 = getSubjectAasString(type, oldAdn);
        String[] aasArr2 = getSubjectAasString(type, newAdn);
        ...
        log("getUpdateSubjectFlag oldCount = " + oldCount
                + " newCount = " + newCount);


        int count = efids.length;
    ...
    private int updateSubjectOfAdn(int type, int num,
        ...
        log("Begin : updateSubjectOfAdn, file type = " + type + ", pbrNum = " + num + ", adnNum = "
                + adnNum + ", index = " + index);
        ...
        updateSubjectFlag = getUpdateSubjectFlag(num, type, oldAdn, newAdn, efids);
        ...
        for (m = 0; m < efids.length; m++) {
            ...
            if (updateSubjectFlag[m] == 0) {
                log("updateSubjectFlag[" + m + "] = 0");
                subjectNums.add(0);
                n++;
                continue;
            }
            ...
            if (mUsimPhoneBookManager.isSubjectRecordInIap(type, num, m)) {
                ...
                if (anrTagMap == null) {
                    log("anrTagMap is null");
                    return -1;
                }

                if (record != null) {
                    if (anrTagMap[m][1] >= record.length) {
                        log("anrTagMap[m][1] Error");
                        return -1;
                    }
                ...
                log("updateSubjectOfAdn subjectNum[m] = " + subjectNum[m]);
                if (subjectNum[m] != -1) {
                    subjectNums.add(subjectNum[m]);
                } else {
                    log("file capacity full");
                    return -1;
                }
                ...
                if (anrTagMap[m][0] > 0) {
                    log("begin to update IAP ---IAP recordId = " + adnNum + ", iapEF = " + Integer.toHexString(iapEF).toUpperCase());
        ...
        for (int i = 0; i < subjectEfids.size(); i++) {
            if (subjectNums.get(i) != 0) {
                ...
                if (type == UsimPhoneBookManagerEx.USIM_SUBJCET_EMAIL) {
                ...
                if (type == UsimPhoneBookManagerEx.USIM_SUBJCET_SNE
                        && TelePhonebookUtils.isSupportOrange()) {
    ...
    public synchronized void updateUSIMAdnBySearch(int efid,
        ...
        for (int num = 0; num < mUsimPhoneBookManager.getNumRecs(); num++) {
            ...
            oldAdnList = getRecordsIfLoadedEx(efid);
            if (oldAdnList == null) {
                if (num == mUsimPhoneBookManager.getNumRecs() - 1) {
                    sendErrorResponse(response,
                            IccPhoneBookOperationException.WRITE_OPREATION_FAILED,
                            "Adn list not exist for EF:0x" + Integer.toHexString(efid).toUpperCase());
                    return;
            ...
            if (find_index) {
                ...
                log("updateUSIMAdnBySearch (3) mInsertId = " + mInsertId);

                oldAdn = oldAdnList.get(index-1);

                //UNISOC: add for bug1174966, insert sim contact with email and fixed number more than 20 digits, email can insert success
                if (!checkAnrLength(newAdn)) {
                    log("[updateUSIMAdnBySearch] Max length of dailing mNumber is 20");
                    sendErrorResponse(response, IccPhoneBookOperationException.OVER_ANR_MAX_LENGTH, "Anr number too long");
                    return;
                }
                ...
                if (updateEmailResult == -1) {
                    //UNISOC: add for bug1145063, delete fail efid from mUserWriteResponse
                    mUserWriteResponse.delete(efid);
                ...
                int updateAnrResult = updateSubjectOfAdn(UsimPhoneBookManagerEx.USIM_SUBJCET_ANR, recNum,
                        adnRecordLoaderEx, mInsertId, index, efid, oldAdn, newAdn, iapEF, pin2, aasIndex);
                log("updateAnrResult = " + updateAnrResult);
                if (updateAnrResult < 0) {
                    ...
                    if (recNum == mUsimPhoneBookManager.getNumRecs() - 1) {
                        ...
                        /** UNISOC: add for bug1169714, SIM card fixed number more than 20 digits, restart lost . @{ */
                        if (updateAnrResult == -1) {
                            sendErrorResponse(response,
                                    IccPhoneBookOperationException.ANR_CAPACITY_FULL, "Anr capacity full");
                        } else {
                            sendErrorResponse(response, IccPhoneBookOperationException.OVER_ANR_MAX_LENGTH, "Anr number too long");
                        }
                        /** @｝ */
                ...
                if (TelePhonebookUtils.isSupportOrange()) {
                    ...
                    if (updateSneResult < 0) {
                        log("update sne failed");
                        mUserWriteResponse.delete(efid);
                        sendErrorResponse(response,
                                IccPhoneBookOperationException.WRITE_OPREATION_FAILED,
                                "sne capacity full");
                ...
                if (newAdn.extRecordIsNeeded()) {
                    ...
                    byte extIndex = (byte) 0xFF;
                    log("oldAdn extIndex = " + oldAdn.mExtRecord);
                    if (oldAdn.mExtRecord != 0xFF) {
                        extIndex = (byte) oldAdn.mExtRecord;
                    } else {
                        extIndex = getAvailableExtIndex(extensionEF, efid);
    ...
    public int updateGasBySearch(String oldGas, String newGas) {
        ...
        int[] gasSize = getRecordsSizeByEf(gasEfId);
        if (gasSize == null) {
            return -1;
        }
    ...
    public synchronized void updateUSIMAdnByIndex(int efid, int simIndex,
       ...
       log("adn efid:0x" + Integer.toHexString(efid).toUpperCase()
        ...
        //UNISOC: add for bug1174966, insert sim contact with email and fixed number more than 20 digits, email can insert success
        if (!checkAnrLength(newAdn)) {
            log("[updateUSIMAdnByIndex] Max length of dailing mNumber is 20");
            sendErrorResponse(response, IccPhoneBookOperationException.OVER_ANR_MAX_LENGTH, "Anr number too long");
            return;
        }
        ...
        if (TelePhonebookUtils.isSupportOrange()) {
            ...
            if (aasArr != null && newAdn.mAas != null) {
                ...
                for (int i = 0; i < aas.length; i++) {
                    aasIndex[i] = aasArr.indexOf(aas[i].toString()) + 1;
                    log("aas[" + i + "] = " + aas[i] + ", aasIndex = " + aasIndex[i]);
                }
            }
        }
        int updateAnrResult = updateSubjectOfAdn(UsimPhoneBookManagerEx.USIM_SUBJCET_ANR, recNum,
                ...
                iapEF, pin2, aasIndex);
        if (updateAnrResult < 0) {
            //UNISOC: add for bug723997, anr size load fail, Usim only support one phone number.
//            sendErrorResponse(response,
//                    IccPhoneBookOperationException.WRITE_OPREATION_FAILED,
//                    "Adn list update fail for EF:0x" + Integer.toHexString(efid).toUpperCase());
        ...
        /** UNISOC: add for bug1062461, An exception is thrown when the email update failed @{ */
        int updateEmailResult = updateSubjectOfAdn(UsimPhoneBookManagerEx.USIM_SUBJCET_EMAIL, recNum,
                adnRecordLoaderEx, mInsertId, adnIndex, efid, oldAdn, newAdn,
                iapEF, pin2, null);
        if (updateEmailResult < 0) {
            log("update email failed");
            mUserWriteResponse.delete(efid);
            sendErrorResponse(response,
                    IccPhoneBookOperationException.EMAIL_CAPACITY_FULL,
                    "Email capacity full");
    ...
    public void handleMessage(Message msg) {
        ...
        case EVENT_UPDATE_USIM_ADN_DONE:
            ...
            if (ar.exception == null && mAdnLikeFiles.get(efid) != null) {
                ...
                Rlog.e(TAG, " fail to Update Usim Adn");
            }
            response = mUserWriteResponse.get(efid);
            mUserWriteResponse.delete(efid);
            if (response != null) {
    ...
    public int updateAasBySearch(String oldAas, String newAas) {
        ...
        int[] aasSize = getRecordsSizeByEf(aasEfId);
        if (aasSize == null) {
            return -1;
        }
    ...
    public int[] getEFAdnRecordSize() {
        return new AdnRecordLoaderEx(mFh).getRecordsSize(EF_ADN);
    }

    //UNISOC: add for bug1174966, insert sim contact with email and fixed number more than 20 digits, email can insert success
    private boolean checkAnrLength(AdnRecordEx newAdn) {
        String[] ret = null;
        String anr = "";
        if (!TextUtils.isEmpty(newAdn.mAnr)) {
            ret = (newAdn.mAnr + "1").split(";");
