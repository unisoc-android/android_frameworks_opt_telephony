...
public class UiccPhoneBookControllerEx extends IIccPhoneBookEx.Stub {
    ...
    @Override
    public int getGroupNum(int subId) throws android.os.RemoteException {
        IccPhoneBookInterfaceManager iccPbkIntMgr =
                getIccPhoneBookInterfaceManager(subId);
        if (iccPbkIntMgr != null) {
            return iccPbkIntMgr.getGroupNum();
        } else {
            Rlog.e(TAG, "getGroupNum iccPbkIntMgr is"
                    + " null for Subscription:" + subId);
            return 0;
        }
    }
    ...
    @Override
    public int[] getUsimGroupSize(int subId) {
        IccPhoneBookInterfaceManager iccPbkIntMgr =
                getIccPhoneBookInterfaceManager(subId);
        int[] usimGroupSize = null;
        if (iccPbkIntMgr != null) {
            usimGroupSize = iccPbkIntMgr.getUsimGroupSize();
        } else {
            Rlog.e(TAG, "getUsimGroupSize iccPbkIntMgr is"
                    + " null for Subscription:" + subId);
        }
        return usimGroupSize;
    }
