...
public class AdnRecordEx implements Parcelable {
    ...
    public byte[] buildExtString() {
        ...
        byte[] extBcdNumber = PhoneNumberUtils.numberToCalledPartyBCD(extNumber, PhoneNumberUtils.BCD_EXTENDED_TYPE_EF_ADN);
    ...
    public byte[] buildAdnString(int recordSize) {
        ...
        if (TextUtils.isEmpty(mNumber) && TextUtils.isEmpty(mAlphaTag)) {
            ...
            if (mSne != null && mSne.length() != 0) {
                throw new IccPhoneBookOperationException(IccPhoneBookOperationException.WRITE_OPREATION_FAILED, "number and alphaTag is null but sne is not null ");
            }
        }
        /* @} */
        ...
        if (TextUtils.isEmpty(mNumber) && TextUtils.isEmpty(mAlphaTag)) {
            ...
            if (!TextUtils.isEmpty(numberNoPlus) && numberNoPlus.length() <= MAX_LENTH_ADN) {
                Rlog.d(LOG_TAG, "mNumber.length is " + numberNoPlus.length());
                ...
                adnString[footerOffset + ADN_CAPABILITY_ID] = (byte) 0xFF; // Capacility Id
                ...
                if (!TextUtils.isEmpty(adnNumber)) {
                    byte[] adnBcdNumber = PhoneNumberUtils.numberToCalledPartyBCD(adnNumber, PhoneNumberUtils.BCD_EXTENDED_TYPE_EF_ADN);
            ...
            if (!TextUtils.isEmpty(mAlphaTag)) {
                try {
                    byteTag = TelePhonebookUtils.stringToGsmAlphaSS(mAlphaTag);
                    System.arraycopy(byteTag, 0, adnString, 0, byteTag.length);
                } catch (EncodeException ex) {
                    try {
                        byteTag = mAlphaTag.getBytes("utf-16be");
                         /* UNISOC: Bug 979503 When utf-16be encoding is used, a byte flag bit is reserved. @{ */
    ...
    public byte[] buildEmailString(int recordSize, int recordSeq, int efid,
        ...
        if (mEmails == null || "".equals(mEmails)) {
            return emailString;
        } else {
        ...
        if (!TextUtils.isEmpty(emailRecord)) {
            try {
                byteTag = TelePhonebookUtils.isAsciiStringToGsm8BitUnpackedField(emailRecord);
                if (byteTag.length > footerOffset) {
                    Rlog.e(LOG_TAG, "emailRecord is overlength");
                    return null;
                }
    ...
    public byte[] buildAnrString(int recordSize, int anrCount, int efid,
        ...
        if (TextUtils.isEmpty(mAnr) || mAnr.equals(";") || mAnr.equals(";;")) {
            ...
            if (TextUtils.isEmpty(anrRecord)) {
                ...
                anrNumber = PhoneNumberUtils.numberToCalledPartyBCD(anrRecord, PhoneNumberUtils.BCD_EXTENDED_TYPE_EF_ADN);
                if (anrNumber == null) {
                    return anrString;
                }
    ...
    public byte[] buildSneString(int recordSize, int recordSeq, int efid, int adnNum) {
        ...
        if (!TextUtils.isEmpty(sneRecord)) {
            try {
                byteTag = TelePhonebookUtils.isAsciiStringToGsm8BitUnpackedField(sneRecord);
                if (byteTag.length > footerOffset) {
                    Rlog.e(LOG_TAG, "sneRecord is overlength");
                    return null;
                }
