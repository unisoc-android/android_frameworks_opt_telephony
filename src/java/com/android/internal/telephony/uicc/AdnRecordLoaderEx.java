...
public class AdnRecordLoaderEx extends Handler {
    ...
    /* UNISOC: PhoneBook for AAS {@ */
    static final int EVENT_UPDATE_AAS_RECORD_DONE = 14;
    /* @} */
    /* UNISOC: PhoneBook for SNE {@ */
        ...
        try {
            switch (msg.what) {
                case EVENT_EF_LINEAR_RECORD_SIZE_DONE:
                    Rlog.d(LOG_TAG,"handle EVENT_EF_LINEAR_RECORD_SIZE_DONE, ef:0x"
                    ...
                    Rlog.i(LOG_TAG, "recordNumber = " + mRecordNumber + ", adn.ext = "
                           + adn.mExtRecord);
                    if(data == null) {
                ...
                case EVENT_ADN_LOAD_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_ADN_LOAD_DONE");
                    ar = (AsyncResult)(msg.obj);
                    data = (byte[])(ar.result);
                ...
                case EVENT_EXT_RECORD_LOAD_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_EXT_RECORD_LOAD_DONE");
                    ar = (AsyncResult)(msg.obj);
                    data = (byte[])(ar.result);
                    adn = (AdnRecordEx)(ar.userObj);
                ...
                case EVENT_ADN_LOAD_ALL_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_ADN_LOAD_ALL_DONE");
                    ar = (AsyncResult)(msg.obj);
                ...
                case EVENT_EXT_LOAD_ALL_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_EXT_LOAD_ALL_DONE");
                    ar = (AsyncResult) (msg.obj);
                ...
                case EVENT_EF_CYCLIC_LINEAR_RECORD_SIZE_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_EF_CYCLIC_LINEAR_RECORD_SIZE_DONE, ef:0x" + Integer.toHexString(mEf));
                    ar = (AsyncResult)(msg.obj);
                    adn = (AdnRecordEx)(ar.userObj);
                    if (ar.exception != null) {
                        throw new IccPhoneBookOperationException(
                   ...
                   if (CyclicrecordSize.length != 3 || mRecordNumber > CyclicrecordSize[2]) {
                    ...
                    Rlog.i(LOG_TAG, "recordNumber = " + mRecordNumber);
                ...
                case EVENT_UPDATE_ANR_RECORD_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_UPDATE_ANR_RECORD_DONE");
                    ar = (AsyncResult) (msg.obj);
                    if (ar.exception != null) {
                        throw new IccPhoneBookOperationException(
                                IccPhoneBookOperationException.WRITE_OPREATION_FAILED,
                ...
                case EVENT_UPDATE_EXT_RECORD_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_UPDATE_EXT_RECORD_DONE");
                    ar = (AsyncResult) (msg.obj);
                ...
                case EVENT_GET_SIZE_DONE:
                    Rlog.d(LOG_TAG, "handle EVENT_GET_SIZE_DONE");
                    ar = (AsyncResult) msg.obj;
                    synchronized (mLock) {
                        if (ar.exception == null) {
                            ...
                            Rlog.d(LOG_TAG, "GET_RECORD_SIZE Size: " + Arrays.toString(mRecordSize));
                        }
                        notifyPending(ar);
                    }
                    break;
                 ...
                 case EVENT_EF_PBR_SNE_LINEAR_RECORD_SIZE_DONE:
                      ...
                      // int[2] is the number of records in the EF file
                      // So int[0] * int[2] = int[1]
                      Rlog.d(LOG_TAG,"EVENT_EF_PBR_SNE_LINEAR_RECORD_SIZE_DONE (1) :  adnNum = " + mAdnNum
                              + ", number = " + Arrays.toString(recordSize));
                      ...
                      for (int i = 0, size = sneEfids.size(); i < size; i++) {
                           Rlog.e(LOG_TAG, "efids.get(" + i + ") = " + sneEfids.get(i) + ", number = " + sneNums.get(i));
    ...
    public void updateEFEmailToUsim(AdnRecordEx adn, ArrayList<Integer> emailEfids,
       ...
       Rlog.i(LOG_TAG, "updateEFEmailToUsim: emailEfids = " +emailEfids
       ...
       for (int i = 0, count = emailEfids.size(); i < count; i++) {
           Rlog.e(LOG_TAG, "efids.get(" + i + ") = " + emailEfids.get(i) + ", number = " + emailNums.get(i) );
    ...
    public void updateEFAnrToUsim(AdnRecordEx adn, ArrayList<Integer> anrefids,
            ...
            String pin2, Message response,int[] size,/* UNISOC: PhoneBook for AAS*/int aasIndex) {
        mUserResponse = response;
        Rlog.e(LOG_TAG, "anrfids = " + anrefids + ", anrefids.size = " + anrefids.size()
                + ", ADNefid = ef:" + mEf);
        byte[] data;
        mFileCount = 0;
        if (size == null) {
        ...
        for (int i = 0, count = anrefids.size(); i < count; i++) {
            Rlog.e(LOG_TAG, "efids.get(" + i + ") = " + anrefids.get(i) + ", number = " + anrNums.get(i));

            if (anrefids.get(i)!=0 && anrNums.get(i)!=0) {
                mFileCount++;
                //UNISOC: modify for bug1169714, SIM card fixed number more than 20 digits, restart lost.
                data = adn.buildAnrString(size[0], anrEfIndex.get(i), mEf,
                        adnNum,/* UNISOC: PhoneBook for AAS*/aasIndex);
    ...
    public void updateEFAdnToUsim(AdnRecordEx adn, int ef, int extensionEF,
        ...
        Rlog.i(LOG_TAG, "recordNumber = " + recordNumber + ", adn.ext = "
                + adn.mExtRecord);
